...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        dp.add_handler(CommandHandler("tasks", self.tasks_command))  # <-- добавили /tasks
    ...
    def start_command(self, update: Update, context: CallbackContext):
        ...
        if user:
            text = _(f"Hello, {user.first_name}! Welcome to the bot menu.")
            reply_markup = self._get_main_menu()
            update.message.reply_text(text, reply_markup=reply_markup)
        else:
            text = _("Hello! You have not linked an account yet.")
            keyboard = [[InlineKeyboardButton(_("Link account"), callback_data=CD_LINK)]]
    ...
    def tasks_command(self, update: Update, context: CallbackContext):
        ...
        if user:
            ...
            update.message.reply_text(
                _("You have not linked an account yet. Use /start.")
            )
    ...
    def echo_message(self, update: Update, context: CallbackContext):
        ...
        update.message.reply_text(_("I am a bot. Use /start to see the menu."))
    ...
    def link_button_handler(self, update: Update, context: CallbackContext):
        ...
        query.edit_message_text(
            text=_("Please enter your email address (or type /cancel to cancel):")
        )
    ...
    def link_email_handler(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            update.message.reply_text(_(f"Successfully linked Telegram with {user.email}!"))
            main_menu = self._get_main_menu()
            update.message.reply_text(_("Now you can use the menu below:"), reply_markup=main_menu)
        except CustomUser.DoesNotExist:
            update.message.reply_text(
                _("User with this email not found. Please try again or type /cancel.")
            )
            return LINKING_EMAIL
    ...
    def cancel_link_handler(self, update: Update, context: CallbackContext):
        update.message.reply_text(_("Linking cancelled. Use /start again if you want."))
        return ConversationHandler.END
    ...
    def button_handler(self, update: Update, context: CallbackContext):
        ...
        if action == CD_MENU:
            if user:
                ...
                query.edit_message_text(
                    text=_("You have not linked an account yet. Use /start.")
                )
            ...
            if not user:
                query.edit_message_text(_("You have not linked an account yet."))
                return
            page = 0
            for p in params:
    ...
    def _get_tasks_menu(self) -> InlineKeyboardMarkup:
        keyboard = [
            ...
            [
                InlineKeyboardButton(_("Tasks for specific Pet (week)"), callback_data=CD_SHOW_PETS),
            ],
            [
    ...
    def _show_today_tasks(self, query, user, page=0):
        ...
        text_lines.append(_(f"Page {page+1} of {total_pages}"))

        for t in tasks_list:
            dt_str = self._format_datetime(t.due_datetime, user)
            text_lines.append(f"- {t.title} ({dt_str})")
        ...
        for t in tasks_list:
            ...
            btn = InlineKeyboardButton(t.title, callback_data=cb_data)
            buttons.append([btn])
    ...
    def _show_pets_list(self, query, user, page=0):
        pets_qs = Pet.objects.filter(owner=user).order_by('name')
        page_size = 8
        start = page * page_size
        end = start + page_size
        ...
        text_lines = []
        text_lines.append(_("Select a Pet (week tasks):"))
        text_lines.append(_(f"Page {page+1} of {total_pages}"))
        for p in pets_list:
    ...
    def _show_week_tasks(self, query, user, pet_id, page=0):
        ...
        try:
            ...
            text_lines.append(_(f"Tasks for pet: {pet_obj.name} (next 7 days)"))
    ...
    def _mark_task_status(self, query, user, task_id, action):
        ...
        if task.status not in [Task.TaskStatus.PLANNED, Task.TaskStatus.OVERDUE]:
            query.edit_message_text(
                _(f"Task already has status {task.status}. Cannot change.")
            )
            return
    ...
    def _mark_task_status_from_notification(self, query, user, task_id, action):
        ...
        if action == CD_NOTIFY_DONE:
            task.mark_as_done(user)
            task.save()
            result_text = _(f"Task '{task.title}' marked as done.")
        else:
            task.mark_as_skipped(user)
            task.save()
            result_text = _(f"Task '{task.title}' marked as skipped.")
